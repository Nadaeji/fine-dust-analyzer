...
def predict_all_cities(season, china_value, pollutant='PM2.5'):
    if pollutant == 'PM2.5':
        svr_models = svr_models_pm25
        scaler = scaler_pm25
    else:
        svr_models = svr_models_pm10
        scaler = scaler_pm10
    
    if season not in svr_models:
        return None
    ...
    predictions = {}
    for city in nearby_cities:
        if city in svr_models[season]:
            prediction = svr_models[season][city].predict(input_scaled)[0]
